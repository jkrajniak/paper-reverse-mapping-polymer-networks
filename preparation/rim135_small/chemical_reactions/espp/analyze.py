...
#cr[:, 1] += np.array([len([b for b in v if -1 not in b]) for v in h5['/connectivity/chem_bonds_1/value']])
...
cv1 = h5['/connectivity/chem_bonds_1']
cl_st = h5['/connectivity/bonds_0']
for t in range(components.shape[0]):
    g = nx.Graph()
    g.add_edges_from(cl_st)
