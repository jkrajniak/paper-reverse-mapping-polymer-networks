...
find ${DIR} -iregex ".*data_s1[0-9]+/.*energy.*" | while read a; do
    fname=`basename $a`
    echo $a
    case=`echo $a | cut -f9 -d'/' | cut -f2 -d_ | tr -d 's'`
    alpha=`echo $fname | cut -f3 -d_`
    cp -v $a energy_${alpha}_${case}.xvg
    #echo $a
done

cp -v /home/teodor/sshfs/vic-staging/mf/mf3/backmapping/data_sfix9/sim0_energy_0.001_86790_first.csv energy_0.001_13.xvg
