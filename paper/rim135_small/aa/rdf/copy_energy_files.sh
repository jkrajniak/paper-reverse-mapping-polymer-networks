...
find ~/sshfs/vic-staging/rim135/backmapping/small/0_100/backmapping_aa -iregex ".*gmx_s[0-9]+/.*/nvt/rdf.*\.xvg" | while read a; do
...
find ~/sshfs/vic-staging/rim135/backmapping/small/0_100/backmapping_aa -iregex ".*gmx_s[0-9]+/.*/nvt/ring-ring.dist.new" | while read a; do
    case=`echo $a | cut -f11 -d'/' | cut -f2 -d_`
    alpha=`echo $a | cut -f12 -d'/'`
